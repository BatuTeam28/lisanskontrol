...
Batu_header = render('+100      BATU', colors=['white', 'red'], align='center')
print("\x1b[1;36m—" * 67)
print(Batu_header)
print("\x1b[1;36m—" * 67)
...
def pppp():
    ...
    be = good_ig          
    print(f"\r          {B}<|  {C1}Hits : {M}{ge}  {E} Bad {HH}: {M}{bt}  {W9}False {HH} : {M}{be}    {R}|> \r", end='')
...
def rest(user):
    try:
        ...
        Batuporno = response.get('email', 'h h h')
    except:
        Batuporno = 'h h h'
    return Batuporno
...
def InfoAcc(username, domain):
    ...
    full_name = account_info.get('full_name', 'Bilinmiyor')
    followers = account_info.get('follower_count', 0)
    following = account_info.get('following_count', 'Bilinmiyor')
    posts = account_info.get('media_count', 'Bilinmiyor')
    bio = account_info.get('biography', '—')
    meta_status = "🟢 METΛ AKTİF" if followers > 99 else "🔴 METΛ PASİF"
    total_hits += 1
...
║ 🧬 𝙈𝙀𝙏𝘼     : {meta_status}
║ 🧪 𝙍𝙀𝙎𝙏     : {rest(username)}
║ 🧮 𝙏𝙊𝙋𝙇𝘼𝙈 𝙃𝙄𝙏 : {total_hits}
╚═════════════════════════════════════════╝
   🛠️ Py By → @BatuX28 | 🔐 Vip Tool™
"""
    try:
...
for _ in range(100):
    Thread(target=Batu_python).start()
